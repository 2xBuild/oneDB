import { eq, and, gte, lt, inArray } from "drizzle-orm";
import { db } from "../db";
import { ideas, likes, comments, users } from "../db/schema";
import { CreateIdeaInput, UpdateIdeaInput } from "../zod";
import { NotFoundError, UnauthorizedError } from "../utils/errors";
import { randomBytes } from "crypto";

export class IdeasService {
  async create(userId: string, data: CreateIdeaInput) {
    const ideaId = randomBytes(16).toString("hex");
    const [idea] = await db
      .insert(ideas)
      .values({
        id: ideaId,
        ...data,
        authorId: userId,
      })
      .returning();

    // Fetching author information
    const userResults = await db
      .select()
      .from(users)
      .where(eq(users.id, userId))
      .limit(1);

    const user = userResults.length > 0 ? userResults[0] : null;
    const author = user ? {
      id: user.id,
      name: user.name,
      email: user.email,
      avatar: user.avatar,
    } : null;

    return {
      ...idea,
      author,
    };
  }

  async findAll(limit = 50, offset = 0, week?: string) {
    let conditions: any[] = [];

    // Weekly filter: week format "YYYY-Www" (e.g., "2024-W01")
    if (week) {
      const [year, weekNum] = week.split("-W");
      if (year && weekNum) {
        const yearNum = parseInt(year);
        const weekNumber = parseInt(weekNum);
        
        // Computing ISO week boundaries
        // Getting January 1st of the year
        const jan1 = new Date(yearNum, 0, 1);
        // Getting day of week (0 = Sunday, 1 = Monday, etc.)
        const dayOfWeek = jan1.getDay();
        // Computing days to first Monday (ISO week starts on Monday)
        const daysToMonday = dayOfWeek === 0 ? -6 : 1 - dayOfWeek;
        const firstMonday = new Date(jan1);
        firstMonday.setDate(jan1.getDate() + daysToMonday);
        
        // Computing start of requested week
        const weekStart = new Date(firstMonday);
        weekStart.setDate(firstMonday.getDate() + (weekNumber - 1) * 7);
        const weekEnd = new Date(weekStart);
        weekEnd.setDate(weekStart.getDate() + 7);
        
        conditions.push(gte(ideas.createdAt, weekStart));
        conditions.push(lt(ideas.createdAt, weekEnd));
      }
    }

    let query = db
      .select()
      .from(ideas);

    if (conditions.length > 0) {
      query = query.where(and(...conditions)) as any;
    }

    const ideaResults = await query
      .limit(limit)
      .offset(offset)
      .orderBy(ideas.createdAt);

    // Getting unique user IDs
    const userIds = [...new Set(ideaResults.map(i => i.authorId))];
    
    // Fetching all users at once
    const userResults = userIds.length > 0
      ? await db
          .select()
          .from(users)
          .where(inArray(users.id, userIds))
      : [];

    // Creating map of users for quick lookup
    const userMap = new Map(userResults.map(u => [u.id, u]));

    // Returning ideas with author info
    return ideaResults.map((idea) => {
      const user = userMap.get(idea.authorId);
      const author = user ? {
        id: user.id,
        name: user.name,
        email: user.email,
        avatar: user.avatar,
      } : null;

      return {
        ...idea,
        author,
      };
    });
  }

  async findById(id: string) {
    const ideaResults = await db
      .select()
      .from(ideas)
      .where(eq(ideas.id, id))
      .limit(1);

    if (ideaResults.length === 0) {
      throw new NotFoundError("Idea not found");
    }

    const idea = ideaResults[0]!;

    // Fetching author information
    const userResults = await db
      .select()
      .from(users)
      .where(eq(users.id, idea.authorId))
      .limit(1);

    const user = userResults.length > 0 ? userResults[0] : null;
    const author = user ? {
      id: user.id,
      name: user.name,
      email: user.email,
      avatar: user.avatar,
    } : null;

    return {
      ...idea,
      author,
    };
  }

  async update(id: string, userId: string, data: UpdateIdeaInput) {
    const idea = await this.findById(id);

    if (idea!.authorId !== userId) {
      throw new UnauthorizedError("You can only update your own ideas");
    }

    // Type assertion to work around Drizzle type inference bug for updatedAt field
    const [updated] = await db
      .update(ideas)
      .set({
        ...data,
        updatedAt: new Date(),
      } as any)
      .where(eq(ideas.id, id))
      .returning();

    if (!updated) {
      throw new NotFoundError("Idea not found");
    }

    // Fetching author information
    const userResults = await db
      .select()
      .from(users)
      .where(eq(users.id, updated.authorId))
      .limit(1);

    const user = userResults.length > 0 ? userResults[0] : null;
    const author = user ? {
      id: user.id,
      name: user.name,
      email: user.email,
      avatar: user.avatar,
    } : null;

    return {
      ...updated,
      author,
    };
  }

  async delete(id: string, userId: string) {
    const idea = await this.findById(id);

    if (idea!.authorId !== userId) {
      throw new UnauthorizedError("You can only delete your own ideas");
    }

    // Permanent delete: deleting associated likes and comments first
    await db.delete(likes).where(and(
      eq(likes.targetType, "idea"),
      eq(likes.targetId, id)
    ));
    await db.delete(comments).where(and(
      eq(comments.targetType, "idea"),
      eq(comments.targetId, id)
    ));
    
    // Then deleting the idea
    await db.delete(ideas).where(eq(ideas.id, id));

    return { id, deleted: true };
  }

  async getUniqueTags() {
    const results = await db
      .select({ tags: ideas.tags })
      .from(ideas);
    
    const allTags = new Set<string>();
    results.forEach((r) => {
      if (r.tags) {
        r.tags.forEach((tag) => allTags.add(tag));
      }
    });
    
    // Getting tag counts to sort by popularity
    const tagCounts = new Map<string, number>();
    results.forEach((r) => {
      if (r.tags) {
        r.tags.forEach((tag) => {
          tagCounts.set(tag, (tagCounts.get(tag) || 0) + 1);
        });
      }
    });
    
    // Sorting by count (descending) then alphabetically
    const uniqueTags = Array.from(tagCounts.entries())
      .sort((a, b) => {
        if (b[1] !== a[1]) return b[1] - a[1]; // Sort by count first
        return a[0].localeCompare(b[0]); // Then alphabetically
      })
      .map(([tag]) => tag);
    
    return uniqueTags;
  }
}

