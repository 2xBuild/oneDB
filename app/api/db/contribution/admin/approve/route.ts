...
export async function POST(request: NextRequest) {
  try {
    ...
    let result;
    if (type === "people") {
      result = await peopleService.approve(id);
    } else if (type === "resource") {
      result = await resourcesService.approve(id);
    } else if (type === "app") {
      result = await appsService.approve(id);
    } else {
      return error("Invalid type", 400);
    }

    return success({ message: "Approved successfully", result });
  } catch (err) {
